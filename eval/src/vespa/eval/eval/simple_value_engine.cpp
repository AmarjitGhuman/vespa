...
using vespalib::tensor::DefaultValueBuilderFactory;
...
{
    return decode_value(input, DefaultValueBuilderFactory::get());
}
...
SimpleValueEngine::map(const Value &a, map_fun_t function, Stash &stash) const
{
    ...
    auto up = GenericMap::perform_map(a, function, DefaultValueBuilderFactory::get());
...
SimpleValueEngine::join(const Value &a, const Value &b, join_fun_t function, Stash &stash) const
{
    ...
    auto up = GenericJoin::perform_join(a, b, function, DefaultValueBuilderFactory::get());
    auto &result = stash.create<std::unique_ptr<Value>>(std::move(up));
    const Value &result_ref = *(result.get());
    return result_ref;
}
...
{
    auto up = GenericMerge::perform_merge(a, b, function, DefaultValueBuilderFactory::get());
    auto &result = stash.create<std::unique_ptr<Value>>(std::move(up));
    const Value &result_ref = *(result.get());
    return result_ref;
}
...
{
    auto up = GenericReduce::perform_reduce(a, aggr, dimensions, DefaultValueBuilderFactory::get());
    auto &result = stash.create<std::unique_ptr<Value>>(std::move(up));
    const Value &result_ref = *(result.get());
    return result_ref;
}
...
{
    auto up = GenericConcat::perform_concat(a, b, dimension, DefaultValueBuilderFactory::get());
    auto &result = stash.create<std::unique_ptr<Value>>(std::move(up));
    const Value &result_ref = *(result.get());
    return result_ref;
}
...
{
    auto up = GenericRename::perform_rename(a, from, to, DefaultValueBuilderFactory::get());
    auto &result = stash.create<std::unique_ptr<Value>>(std::move(up));
    const Value &result_ref = *(result.get());
    return result_ref;
}
